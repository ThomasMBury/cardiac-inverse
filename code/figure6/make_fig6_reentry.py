...
Make figure 6
Stacked linear regression plot for reentry model simulation
...
#-----------
# Simulate reentry model
#------------

# Get parameters
tlag = (df_vv['VV']-df_vv['VN']).mean()
...
def model_reentry(vn):
    vv = 1*vn + tlag + sigma*np.random.normal(0,1)
    return vv


df_sim['VV'] = df_sim.apply(
    lambda row: model_reentry(row['VN']), 
    axis=1)
